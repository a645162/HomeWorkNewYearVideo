...
OpenCLProgram CLCreateProgramImageMirror(cl_context context, cl_device_id device) {
    return {
            context,
            device,
            "ImageMirror",
...
//    cv::cvtColor(image1, image1, cv::COLOR_BGRA2GRAY);
    ...
    OpenCLProgram program_mirror = CLCreateProgramImageMirror(context, device);
