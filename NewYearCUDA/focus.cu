...
void MaskImageCircle(
        ...
        const float m_1 = 0, const float m_2 = 0, const float max_distance = 0,
        const uchar3 focus_color = {0, 0, 0},
        const uchar color_alpha = 150
) {
    ...
    if (x < width && y < height) {
        ...
        if (distance > static_cast<float>(radius)) {
            ...
            if (focus_lamp) {
                const auto x_1 =
                        (m_1) * static_cast<float>(y - light_source_y) + static_cast<float>(light_source_x);
                const auto x_2 =
                        (m_2) * static_cast<float>(y - light_source_y) + static_cast<float>(light_source_x);
...
void processImageCuda(
        ...
        const int centerX, const int centerY, float radius
) {
...
//    );
    ...
    printf("@outputVideo.isOpened() %d\n", outputVideo.isOpened());
...
//    cv::Mat frame(frameSize, CV_8UC4);
    ...
    for (int i = 0; i < TOTAL_FRAME; i++) {
        ...
        auto centerX = static_cast<int>(
                ...
                * (static_cast<float>(i) / static_cast<float>(TOTAL_FRAME))
        );
